...
> **데모 안내**
> 현재 대기질 API 이슈로 인해 미세먼지 수치는 실시간 값이 아닌 **데모용 데이터**를 기반으로 표시됩니다.
> 위치 정보와 미세먼지 수치 등은 현재 **기본 설정값**이 적용되어 있습니다.
> 핵심 기능(행동 가이드, UI 흐름, 상호작용) 확인을 위한 데모 버전입니다.
...
- **대기질 모니터링 (데모)**: 현재는 API 이슈로 실시간 값 대신 데모 데이터를 기반으로 PM10, PM2.5 농도와 등급을 표시하며, 상황에 맞는 대응 방안을 제시합니다.
...
- **미세먼지 데이터 (데모)**: API 이슈로 현재는 데모 데이터 기반 PM10, PM2.5 농도 및 등급 표시
...
아래 순서대로 실행하면 로컬 개발 서버를 바로 확인할 수 있습니다.
...
배포 전 빌드 결과를 확인하려면 아래 명령어를 사용합니다.
...
> **현재 상태**
> API 이슈로 인해 README 작성 시점 기준 서비스에 표시되는 미세먼지 수치는 **실시간 관측값이 아닌 데모 데이터**입니다.
> 위치 정보와 미세먼지 수치 등은 데모용 **기본 설정값**으로 제공됩니다.

- **한국환경공단 대기질 공공데이터**
  - 원래는 실시간 미세먼지(PM10, PM2.5) 농도 데이터 조회에 사용
  - 측정소별 데이터 조회
  - 스크립트를 통해 주기적으로 데이터 업데이트 가능

- **에어코리아 Excel 데이터**
  - API 서버 불안정/이슈 시 사용하는 대체용(데모) 데이터
